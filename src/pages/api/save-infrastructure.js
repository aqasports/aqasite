...
export async function POST({ request }) {
  try {
    ...
    const filePath = path.join(process.cwd(), 'src', 'data', 'infrastructure.json');
    fs.writeFileSync(filePath, JSON.stringify(data.infrastructures, null, 2), 'utf8');

    return new Response(JSON.stringify({ success: true, message: 'Infrastructures enregistrées avec succès' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (err) {
